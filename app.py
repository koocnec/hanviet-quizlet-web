...
st.set_page_config(page_title="Bùi Văn Toàn V12", page_icon="📁", layout="wide")
...
st.caption("Bản V12: Có thêm form nhập thẻ giống OpenQuiz và ô từ đồng nghĩa.")
...
with st.sidebar:
    ...
    st.caption("File thường là: B = tiếng Hàn, A = nghĩa Việt, C = giải thích, D = từ đồng nghĩa.")
...
try:
    ...
    loaded_cards = make_cards(df, kr_col, vi_col, detail_col, synonym_col)

    if not loaded_cards:
        st.error("Không đọc được thẻ. Cột tiếng Hàn đang trống hoặc bạn chọn sai cột tiếng Hàn.")
        st.stop()

    data_key = f"{source}|{google_url}|{getattr(uploaded, 'name', '')}|{kr_col}|{vi_col}|{detail_col}|{synonym_col}|{len(loaded_cards)}"

    if st.session_state.editor_data_key != data_key:
        st.session_state.editor_data_key = data_key
        st.session_state.editor_cards = [dict(x) for x in loaded_cards]
        st.session_state.editor_i = 0

    cards_all = st.session_state.editor_cards
    ...
    with tab_input:
        st.subheader("✍️ Nhập thẻ giống OpenQuiz")
        ...
        with top_right:
            st.caption("Nhập nội dung thẻ học")

        col_a, col_b, col_img = st.columns([5, 5, 1])
        ...
        with col_img:
            st.write("")
            st.write("")
            delete_top = st.button("🗑️", key=f"delete_top_{edit_i}", use_container_width=True)
        ...
        with col_c:
            new_pronunciation = st.text_input(
                "Phát âm (Tùy chọn)",
                value=edit_card.get("pronunciation", ""),
                placeholder="VD: /haːlou/",
                key=f"editor_pronunciation_{edit_i}"
            )

        with col_d:
            new_type = st.text_input(
                "Loại từ (Tùy chọn)",
                value=edit_card.get("word_type", ""),
                placeholder="VD: noun, verb...",
                key=f"editor_type_{edit_i}"
            )
        ...
        delete_clicked = btn3.button(
            "🗑️ Xóa thẻ",
            use_container_width=True,
            disabled=len(st.session_state.editor_cards) <= 1
        )

        if delete_clicked or delete_top:
            if len(st.session_state.editor_cards) > 1:
        ...
        st.info("Dữ liệu trong tab này được lưu tạm trong web. Muốn giữ lại lâu dài, hãy tải CSV rồi đưa vào Google Sheets.")

        export_df = pd.DataFrame(st.session_state.editor_cards)
        export_csv = export_df.to_csv(index=False).encode("utf-8-sig")

        st.download_button(
            "⬇️ Tải CSV đã nhập / đã sửa",
            export_csv,
            file_name="tu_vung_da_them_dong_nghia.csv",
            mime="text/csv",
            use_container_width=True
        )
    ...
    with tab_flash:
        ...
        synonym_html = (
            f"<div class='synonyms'><b>Đồng nghĩa:</b> {html.escape(card.get('synonyms', ''))}</div>"
    ...
    with tab_learn:
        ...
        learn_synonym_html = (
            f"<div class='synonyms'><b>Đồng nghĩa:</b> {html.escape(c.get('synonyms', ''))}</div>"
    ...
    with tab_quiz:
        ...
        valid_for_quiz = [x for x in cards if x.get("vi") != "Chưa có nghĩa"]
        ...
        if len(valid_for_quiz) < 4:
            ...
            def make_new_quiz_question():
                ...
                new_wrong_pool = [x for x in valid_for_quiz if x.get("vi") != new_q.get("vi")]

                new_options = [new_q.get("vi", "")] + [
                    x.get("vi", "") for x in random.sample(new_wrong_pool, min(3, len(new_wrong_pool)))
                ]
            ...
            if st.session_state.get("quiz_last_result") == "correct":
                ...
                st.error(f"Sai. Đáp án đúng: {format_expected_answer(q.get('vi', ''), q.get('synonyms', ''))}")
    ...
    with tab_speaking:
        ...
        if not MIC_AVAILABLE:
            ...
            synonym_html = (
                f'<div class="synonyms"><b>Đồng nghĩa:</b> {html.escape(sc.get("synonyms", ""))}</div>'
    ...
    with tab_match:
        ...
        valid_for_match = [x for x in cards if x.get("vi") != "Chưa có nghĩa"]
        ...
        if len(valid_for_match) < 2:
            ...
            rights = [x.get("vi", "") for x in sample]
            score = 0
